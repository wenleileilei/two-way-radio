...
void SX1276FskInit( void )
{
    ...
    SX1276->RegOpMode = RF_OPMODE_MODULATIONTYPE_FSK | RF_OPMODE_SLEEP | RF_OPMODE_FREQMODE_ACCESS_LF;
...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_SYNC:
        if( ( DIO4 == 1 ) && ( PreambleDetected == false ) )// Preamble	( DIO4 == 1 ) &&
        {
            PreambleDetected = true;
            Preamble2SyncTimer = GET_TICK_COUNT( );
        }
        if( ( DIO2 == 1 ) && ( PreambleDetected == true ) && ( SyncWordDetected == false ) ) // SyncAddr ( DIO2 == 1 ) && 
        {
